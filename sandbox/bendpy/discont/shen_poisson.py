#
# Solve -u`` = f in (-1, 1) with u(-1) = u(1) = 0   [1]
# 
...
def solve_shen(g, h, n):
    ...
    x = Symbol('x')
    b = np.array([quad(lambdify(x, g*v), [-1, 0]) for v in shen.shen_basis(n)])
    b += np.array([quad(lambdify(x, h*v), [0, 1]) for v in shen.shen_basis(n)])
...
if __name__ == '__main__':
    ...
    from lega.shen_basis import shen_function
